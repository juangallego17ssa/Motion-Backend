...
export default function FriendsCard({ user, onFetchFollowing, onFetchFriends }) {
  ...
  const postUser = async () => {
    const config = {
      ...
      url: `((process.env.NODE_ENV==="development") ? "http://localhost:8001/" : "https://motion-t3.propulsion-learn.ch/") + \`backend/api/social/followers/toggle-follow/${user.id}/`,
  ...
  const sendFriendRequest = async () => {
    ...
    var config = {
      method: 'post',
      maxBodyLength: Infinity,
      url: ((process.env.NODE_ENV==="development") ? "http://localhost:8001/" : "https://motion-t3.propulsion-learn.ch/") + `backend/api/social/friends/request/${user.id}/`,
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json'
      },
